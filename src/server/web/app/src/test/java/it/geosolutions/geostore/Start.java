...
public class Start {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Use this to set a limit on the number of threads used to respond
			// requests
			 ...
			 * cm.setConstraint(constraint); cm.setPathSpec("/*");
			 * 
			 * SecurityHandler sh = new SecurityHandler(); sh.setUserRealm(new
			 * HashUserRealm("MyRealm","/Users/jdeolive/realm.properties"));
			...
			wah.getServletContext().getContextHandler()
					.setMaxFormContentSize(1024 * 1024 * 2);

			String jettyConfigFile = System.getProperty("jetty.config.file");
			if (jettyConfigFile != null) {
				log.info("Loading Jetty config from file: " + jettyConfigFile);
				(new XmlConfiguration(new FileInputStream(jettyConfigFile)))
						.configure(jettyServer);
			}
			 ...
			 * gracefully terminate the jetty server and shut down the JVM. This
			 * way we can invoke the shutdown hooks while debugging in eclipse.
			 * Can't catch CTRL-C to emulate SIGINT as the eclipse console is
			 * not propagating that event
			 */
