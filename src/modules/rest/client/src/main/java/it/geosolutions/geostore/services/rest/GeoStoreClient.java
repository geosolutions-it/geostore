...
public class GeoStoreClient {
    ...
    public ShortResourceList searchResources(SearchFilter searchFilter) {
        ShortResourceList resourceList =
                getBaseWebResource("resources", "search")
    ...
    public String getData(Long id, MediaType acceptMediaType) {
        return getBaseWebResource("data", id).accept(acceptMediaType).get(String.class);
    }
    ...
    public Category getCategory(Long id) {
        Category category =
                getBaseWebResource("categories", "category", id)
                        .header("Content-Type", MediaType.TEXT_XML)
                        .accept(MediaType.TEXT_XML)
                        .get(Category.class);
    ...
    public Long getCategoryCount(String nameLike) {
        String count =
                getBaseWebResource("categories", "count", nameLike)
                        .accept(MediaType.TEXT_PLAIN)
                        .get(String.class);
    ...
    protected WebResource getBaseWebResource(Object... path) {
        ...
        for (Object o : path) {
            String p = o.toString();
            if (fullpath.charAt(fullpath.length() - 1) != '/') fullpath.append('/');
            fullpath.append(p);
        }
